...
public class KMostFreqElements {
    ...
    public int[] topKFrequent(int[] nums, int k) {
        ...
        Arrays.stream(nums).forEach(i -> freqMap.put(i, freqMap.getOrDefault(i, 0) + 1));

        Main.println(freqMap);
    ...
    public static int freq(int[] nums, int i) {
        int res = (int) Arrays.stream(nums).filter(j -> j == i).count();
        Main.println(i + " found " + res + " times");
        return res;
    }

    public int[] topKFrequent_withHeap(int[] nums, int k) {
        // O(1) time
        if (k == nums.length) {
            return nums;
        }
    ...
    public static void main(String[] args) {
        ...
        KMostFreqElements instance = new KMostFreqElements();
        ...
        // [-12,-68,73,-55,-9,5,75]
        Main.println(Arrays.toString(instance.topKFrequent(ints, 7)));
    }
